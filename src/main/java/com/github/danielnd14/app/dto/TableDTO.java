...
public final class TableDTO {
	...
	public TableDTO(String col, String formula, String rows) {
		...
		this.rows = rows;
		if (validator == null) validator = Validator.instance();
	}
	...
	public String formula(int actualRow) {
		...
		if (Pattern.compile("^\\d+$").matcher(rows).find() && Integer.parseInt(rows) != actualRow) {//singleNumber ex."91"
			...
			if (matcher.find()) {
				...
				boolean isPresent = Arrays.stream(numbers).anyMatch(s -> s.equalsIgnoreCase(actualRow + ""));
	...
	public boolean isValid() {
		return (col != null && formula != null && rows != null) &&
				validator.getColumnValidator().valid(col) &&
				!formula.isBlank() &&
				validator.getRowValidator().valid(rows);
	}
